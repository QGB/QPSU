...
def http_get(u):
	print(U.stime(),u)
	b=N.curl(u,proxies='127.0.0.1:21080')
	return b
...
class Backtest(object):
	...
	def _download_data_to_db(self) -> None:
		data_url = f"https://public.bybit.com/trading/{self.symbol}/";print(data_url)#qgb
		# res = requests.get(data_url)
		# if res.status_code != 200:
			# raise Exception(f"requests error {res.status_code} {res.json()}")
		# soup = BeautifulSoup(res.text, "lxml")
		soup = BeautifulSoup(http_get(data_url).decode('utf-8'), "lxml")
		interval = "5S"
