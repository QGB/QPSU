...
copyfile(base_path + "Default/Cookies", './Cookies')

# Load encryption key
encrypted_key = None
with open(base_path + "Local State", 'r') as file:
...
def get(show_encrypted_value=False):
	...
	# 		(decrypted_value, host_key, name));
		row=[decrypted_value, host_key, name]
		if show_encrypted_value:
