...
def bytesToHex(a):
	...
	return r
def hexToBytes(a):
	if type(a)!=type('') or len(a)%2!=0:return None
	a=a.upper();r=''
...
def list(ap='.',r=False,type='',t='',d=False,dir=False,f=False,file=False):
	...
	if 'd' in type:d=True
	if 'f' in type:f=True
	if type.startswith('a'):d=True;f=True
	...
	if d or dir:r.extend(r3[1])
	if f or file:r.extend(r3[2])
