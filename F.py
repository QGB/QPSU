...
################################
def get_newest_file(p,recent=''):
    U,T,N,F=py.importUTNF()
    f_list=[os.path.join(p,f)for f in os.listdir(p)if os.path.isfile(os.path.join(p,f))]
    if not f_list:return ''
    f=max(f_list,key=lambda x:os.path.getmtime(x))
    if recent:
        now=U.timestamp()
        unit={'s':1,'m':60,'h':3600,'d':86400}[recent[-1]]
        delta=int(recent[:-1])*unit
        if os.path.getmtime(f)<now-delta:return ''
    print(F.ll(f,return_list=1)[0])
    return f
get_dir_new_file=get_newest_file
# get_newest_file(r'C:\Users\Administrator\Pictures\\')
    ...
    with zipfile.ZipFile(zip_filename, 'w') as zipMe:
        for file in fs:
            if os.path.islink(file):
                ...
                if only_filename:
                    if py.istr(only_filename):
                        zipMe.write(file,arcname=T.sub(file,only_filename,),compress_type=zipfile.ZIP_DEFLATED)
                    else:    
