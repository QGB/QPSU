...
def write(file,data,mod='wb',mkdir=False,autoArgs=True):
	py.importU()
	try:
		if autoArgs:
			if  py.len(file)>py.len(data)>0:
				file,data=data,file
...
def read(file,mod='r',returnFile=False):
	'''if returnFile:
			return content,f.name
			'''
	try:
		f=open(autoPath(file),mod)
		s=f.read()
		f.close()
		if returnFile:
			return s,f.name
...
def list(ap='.',type='',t='',r=False,d=False,dir=False,f=False,file=False):
	...
	if not U.inMuti(ap,'/','\\',f=str.endswith):
		if isDir(ap):
			ap+='/'
		else:
			return [abs(ap)]
...
def isFileName(a):
	...
	py.importU()
	return U.all_in(list(a),T.PATH_NAME)
